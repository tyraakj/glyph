    ...
    async start(controller) {
      ...
      try {
        ...
        while (!isStreamClosed) {
          // Upstash redis.get automatically parses JSON if it is a JSON string,
          // but we typed it as `any` because it returns the parsed object if it was saved as JSON
          const statusObj = await redis.get<any>(`design:status:${runId}`);
